...
def internal_nodes_after_contraction(confidence_threshold):
    """
    ...
    for filename in natsorted(os.listdir("RAxML_Files")):
        ...
        if os.path.splitext(filename)[0] == "RAxML_bipartitions":
            ...
            num_internal_nodes_i, num_internal_nodes_f = contraction_threshold(filename, confidence_threshold)
...
def double_line_graph_generator(list1, list2, xlabel, ylabel, name):
    """
...
if __name__ == '__main__':  # if we're running file directly and not importing it
    ...
    internal_nodes_i, internal_nodes_f = internal_nodes_after_contraction(confidence_threshold)
    xlabel = "Window Indices"
    ylabel = "Number of Internal Nodes"
    name = "ContractedGraph. png"

    double_line_graph_generator(internal_nodes_i, internal_nodes_f, xlabel, ylabel, name)
