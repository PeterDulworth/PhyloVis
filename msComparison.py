...
statisticsCalculations = sc.StatisticsCalculations()

def sites_to_newick_ms(input_file):
    """
...
def sites_to_newick_rax(rax_dir, window_size, window_offset):
    """
...
def ms_rax_difference(sites_to_newick_ms_map,  sites_to_newick_rax_map):
    """
    ...
    for i in range(num_sites):
        ...
        if (i in sites_to_newick_ms_map) and (i in sites_to_newick_rax_map):
            ...
            w_rf, uw_rf = statisticsCalculations.calculate_robinson_foulds(ms_newick, rax_newick, True)
...
input_file = "fakeMS.txt"
sites_to_newick_ms_map = sites_to_newick_ms(input_file)

rax_dir = "RAxML_Files"
window_size, window_offset = 10, 10
sites_to_newick_rax_map = sites_to_newick_rax(rax_dir, window_size, window_offset)

sites_to_difference_w, sites_to_difference_uw = ms_rax_difference(sites_to_newick_ms_map,  sites_to_newick_rax_map)

statisticsCalculations.stat_scatter(sites_to_difference_w, "WRFdifference.png", "Difference Between MS and RAxML Output", "Sites Indices", "Weighted Robison-Foulds Distance")
statisticsCalculations.stat_scatter(sites_to_difference_uw, "UWRFdifference.png", "Difference Between MS and RAxML Output", "Sites Indices"," Unweighted Robinson-Foulds Distance")
